...
//  BaseVPN
//
//  Created by Lika Vorobeva on 16.06.2021.
//
...
final public class TunnelsService {
    ...
    func startXray(
        ...
        let tunnelProviderManager = tunnels.last?.tunnelProvider ?? NETunnelProviderManager()
        tunnelProviderManager.localizedDescription = ClientConstants.xRayKey
        tunnelProviderManager.protocolConfiguration = {
            let configuration = NETunnelProviderProtocol()
            configuration.providerBundleIdentifier = Bundle.providerBundleIdentifier
            configuration.serverAddress = ClientConstants.xRayKey
        ...
        tunnelProviderManager.saveToPreferences { [weak self] error in
            ...
            if isTunnelConfigurationChanged {
                if tunnel.status == .connected || tunnel.status == .connecting || tunnel.status == .reasserting {
                    ...
                    tunnel.tunnelProvider.providerSession?.stopTunnel()
                }
            }
